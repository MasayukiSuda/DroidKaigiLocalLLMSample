...
class LiteRTRepository @Inject constructor(
    ...
    override suspend fun initialize() {
        ...
        withContext(Dispatchers.IO) {
            try {
                // Ensure Gemma3 model files exist under filesDir by copying from assets on first run
                val destDir = File(context.filesDir, "models/lite_rt/gemma3")
                if (!destDir.exists()) destDir.mkdirs()
                copyAssetsRecursively(context.assets, "models/lite_rt/gemma3", destDir)

                // Find a .tflite model file (first one)
                val modelFile = destDir.listFiles()?.firstOrNull { it.extension.equals("tflite", ignoreCase = true) }
                ...
                if (modelFile != null && modelFile.exists()) {
                    ...
                    // No model asset placed → fallback to mock
                    isInitialized = true
                    modelSize = 512f // Mock size
    ...
    private suspend fun generateResponse(prompt: String, onToken: suspend (String) -> Unit) {
        ...
        withContext(Dispatchers.IO) {
            // 現時点では Gemma3 用の SentencePiece 未実装のため常にモックへフォールバック
        ...
        try {
            ...
            for (name in assets) {
                ...
                if (children != null && children.isNotEmpty()) {
                    ...
                    copyAssetsRecursively(assetManager, childAssetPath, subDir)
                } else {
                    // File
