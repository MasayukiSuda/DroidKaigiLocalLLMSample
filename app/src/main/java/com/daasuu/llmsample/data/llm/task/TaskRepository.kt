...
class TaskRepository @Inject constructor(
    ...
    override suspend fun release() { isInitialized = false }
    ...
    override suspend fun generateChatResponse(prompt: String): Flow<String> = channelFlow {
        ...
        if (!isInitialized) { send("Error: .task model not found"); return@channelFlow }
    ...
    override suspend fun summarizeText(text: String): Flow<String> = channelFlow {
        if (!isInitialized) initialize()
        if (!isInitialized) { send("Error: .task model not found"); return@channelFlow }
        val full = withContext(Dispatchers.IO) { runMediaPipeGenerate("要約してください: \n\n$text") }
        send(full ?: ("- 要約(擬似): " + text.take(40) + "..."))
    }

    override suspend fun proofreadText(text: String): Flow<String> = channelFlow {
        if (!isInitialized) initialize()
        if (!isInitialized) { send("{}"); return@channelFlow }
        val full = withContext(Dispatchers.IO) { runMediaPipeGenerate("次の文を日本語で最小限に校正してJSONのみで返して: $text") }
    ...
    private fun tryInitMediaPipeGenerator() {
        ...
        for ((genCls, optCls, baseCls) in candidates) {
            try {
                ...
                run {
                    val byFile = genClass.methods.firstOrNull { it.name == "createFromFile" && it.parameterTypes.size == 2 }
                    if (byFile != null) {
                        try {
                            instance = byFile.invoke(null, context, modelPath)
                            if (instance == null) instance = byFile.invoke(null, context, assetRelative)
                        } catch (t: Throwable) {
                ...
                val baseOptionsClass = if (baseCls != null) try { Class.forName(baseCls) } catch (_: Throwable) { null } else null
                if (baseOptionsClass == null) {
                    ...
                    try {
                        ...
                        val setModelPath2 = optBuilder.javaClass.methods.firstOrNull { it.name == "setModelPath" && it.parameterTypes.contentEquals(arrayOf(String::class.java)) }
                        val setModelAssetPath2 = optBuilder.javaClass.methods.firstOrNull { it.name == "setModelAssetPath" && it.parameterTypes.contentEquals(arrayOf(String::class.java)) }
                        if (setModelPath2 != null) setModelPath2.invoke(optBuilder, modelPath)
                        if (setModelAssetPath2 != null) setModelAssetPath2.invoke(optBuilder, assetRelative)
                ...
                val setModelPath = baseBuilder.javaClass.methods.firstOrNull { it.name == "setModelPath" && it.parameterTypes.contentEquals(arrayOf(String::class.java)) }
                val setModelAssetPath = baseBuilder.javaClass.methods.firstOrNull { it.name == "setModelAssetPath" && it.parameterTypes.contentEquals(arrayOf(String::class.java)) }
                ...
                val optionsClass = Class.forName(optCls)
                val optBuilder = optionsClass.getMethod("builder").invoke(null)
                val setBaseOptions = optBuilder.javaClass.methods.firstOrNull { it.name == "setBaseOptions" && it.parameterTypes.size == 1 }
    ...
    private fun runMediaPipeGenerate(prompt: String): String? {
        ...
        return try {
            val m = gen.javaClass.methods.firstOrNull { (it.name == "generate" || it.name == "generateResponse") && it.parameterTypes.size == 1 && it.parameterTypes[0] == String::class.java }
            val result = m?.invoke(gen, prompt)
            when (result) {
                is String -> result
                else -> {
                    val getText = result?.javaClass?.methods?.firstOrNull { it.name == "getText" && it.parameterTypes.isEmpty() }
                    val getOut = result?.javaClass?.methods?.firstOrNull { it.name == "getOutputText" && it.parameterTypes.isEmpty() }
